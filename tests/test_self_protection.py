"""Vigil must not offer to clean itself up.

%APPDATA%/Vigil holds config.json, the scan sessions and the logs. Left to the
generic passes those classify like anyone else's data — "logs" as a log folder,
"cache" as a cache folder, both Safe and both recycle-able — so Vigil could
...
def fake_appdata(monkeypatch, tmp_path):
    ...
    return "c:/users/u/appdata/roaming/vigil"
...
def _vigil_tree():
    ...
    base = "C:/Users/u/AppData/Roaming/Vigil"
    out = [
...
@pytest.mark.parametrize("path", [
    "C:/Users/u/AppData/Roaming/Vigil",
    "C:/Users/u/AppData/Roaming/Vigil/sessions/last_run.json",
    "C:/Users/u/AppData/Roaming/Vigil/logs",
    r"C:\Users\u\AppData\Roaming\Vigil\cache",   # backslashes too
])
...
@pytest.mark.parametrize("path", [
    "C:/Users/u/AppData/Roaming/VigilSomethingElse",   # prefix, not a parent
    "C:/Users/u/AppData/Roaming/Code",
    "C:/Users/u/Downloads/vigil.zip",
    "",
])
...
def test_the_data_folder_becomes_one_protected_entity(fake_appdata):
    entities = ed.detect_entities(_vigil_tree(), "C:/", log_fn=lambda _m: None)
...
def test_the_logs_and_cache_inside_are_not_offered_separately(fake_appdata):
    ...
    strays = [e for e in entities
              if e.path.replace("\\", "/").lower().startswith(fake_appdata + "/")]
    assert not strays, f"offered Vigil's own subfolders: {[e.path for e in strays]}"
...
@pytest.mark.parametrize("path", [
    "C:/Users/u/AppData/Roaming/Vigil",
    "C:/Users/u/AppData/Roaming/Vigil/sessions/session_b7ff978b.json",
])
def test_cleanup_refuses_vigils_own_paths(fake_appdata, path):
...
def test_recycling_a_self_path_is_skipped_not_attempted(fake_appdata, tmp_path):
    ...
    # Treat the temp dir as Vigil's install root for this test.
    ...
    assert victim.exists(), "deleted a file belonging to Vigil"
...
def test_the_user_is_told_what_this_is(qapp):
    ...
    assert status
    assert "Vigil" in recommendation
    assert "finish" in recommendation.lower()
    assert "Vigil" in evidence
...
def test_a_normal_protected_item_keeps_the_plain_wording(qapp):
    ...
    assert "Vigil" not in recommendation
