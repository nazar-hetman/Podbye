...
Output: `dist\Vigil\` — **ship the whole folder**, not just the .exe.

## Why one folder and not one file

This is a licensing constraint, not a packaging preference. Vigil links Qt
...
`dist\Vigil\_internal\PySide6\*.dll` are ordinary files a recipient can
...
- `LICENSE`, `THIRD-PARTY-NOTICES.md` and `licenses\*.txt` must be present in
  `dist\Vigil\_internal\`. The spec bundles them; verify after any spec edit.
...
Zip `dist\Vigil\` and hand over the archive. There is no installer yet, and the
