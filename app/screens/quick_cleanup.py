...
class _SelectionCheckBox(QCheckBox):
    """Minimal tactical checkbox with an explicit visible tick."""
    ...
    def paintEvent(self, event):
        ...
        if checked:
            pen = QPen(QColor(tick if enabled else disabled), 2)
            painter.setPen(pen)
...
class QuickCleanupScreen(QWidget):
    ...
    def _build_ui(self):
        ...
        self._sel_badge = Badge("0 selected", "info")
        self._sel_badge.setFixedWidth(136)
    ...
    def _update_summary(self):
        ...
        if total_bytes >= 1_073_741_824:
            ...
            num_str, unit_str = f"{total_bytes / 1_048_576:.0f}", "MB"
        else:
            num_str, unit_str = f"{total_bytes / 1024:.0f}", "KB"
    ...
    def _on_cleanup_done(self, result):
        ...
        if freed >= 1_073_741_824:
            self._total_num.setText(f"{freed / 1_073_741_824:.1f}")
            self._total_unit.setText("GB")
        elif freed >= 1_048_576:
            self._total_num.setText(f"{freed / 1_048_576:.0f}")
            self._total_unit.setText("MB")
        else:
            self._total_num.setText(f"{freed / 1024:.0f}")
            self._total_unit.setText("KB")
